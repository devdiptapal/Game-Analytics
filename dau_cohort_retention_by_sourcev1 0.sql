...
select distinct  user_uid, dau_date::date, source, 'dau_cohort'
...
select distinct user_uid as user_uid,  dau_date as date, 'dau'
from s_zt_dau
where game_id in  ($gameid$)
and sn_id in  ($snid$)
and client_id in ($clientid$)
...
select a.date::date as 'dau Date',
